...
#import pandas as pd
tweet_sequence=["Delhi COVID Corona verified","Delhi COVID verified", "Delhi verified Oxygen", "Delhi verified bed", "Delhi verified ICU", "Delhi verified Plasma", "Delhi verified blood","Delhi verified Remdesivir"]

#Please visit https://developer.twitter.com/en/apply-for-access, make a developer account here to get the following 4 values
tweet_cap = 500
...
while True:
    try:
        if flag==0:
            twitterStream.filter(track=tweet_sequence, is_async=True)
            flag=1
        if((tweet_count == tweet_cap) and (flag == 1)):
            print("Tweet Count is " + str(tweet_count) + "\nChild Process id: " + str(child_pid))
            flag=flag+1
            os.system("mutt -s \"Tweet Extractor script has capped at " + str(tweet_cap) + " tweets and halted!!\" -a Tweets_Delhi.csv -a Retweets_Delhi.csv -- pinakw@cadence.com,pshirur@cadence.com,joshih@cadence.com < template.txt")
